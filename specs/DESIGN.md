...
Launcher 输入区提供 Chat / Record 两种明确模式。Record mode 的文字提交创建 text Record，录音动作创建 audio Record 并立即打开详情；两种模式的输入卡外框必须严格等高，工具栏新增文字动作不得撑高容器。模式切换不改变全局侧栏或顶部 Tab 结构。任务中心左栏继续以“想法”命名，输入框提交按钮左侧同样提供“开始录音”；从此处开始必须新建 Record Tab 并保留任务中心 Tab 原状。该栏以一个 Record 列表统一展示 text/audio，不再把音频拆成第二个历史页；批量删除前明确展示所选数量与不可恢复后果。

text/audio Record 卡片使用同一信息层级和尺寸语言。audio 卡片第一行左侧复用 text Record 的相对时间 / 自然日期格式，时间语义仍取录音开始时间；右侧始终显示更多菜单，并在 hover/focus 时于更多菜单左侧出现“AI 讨论”快捷动作。第二行显示与标题字号匹配的小型录音图标、单行标题和右侧时长，不在列表重复显示录音/转录状态 Tag。点击“AI 讨论”后先选择 Agent 工作区，再直接打开新的讨论 Session；该主动作不在更多菜单中重复。audio 更多菜单只保留多选、归档与删除，text 更多菜单只保留编辑、多选、归档与删除；列表不提供紧凑播放，播放统一进入详情。卡片内容和动作必须收敛在 Record 列表栏内，虚拟列表留白由 item 自身承担，不得以 hover 按钮或滚动根节点 padding 扩大宽度、溢出左右边界。
...
- 宽窗使用完整左右两栏：左栏顶部依次放标题/状态、录音或播放控制和转写；右侧现场笔记从页面内容顶端开始并独立滚动。标题只占左栏宽度，hover 与 inline 编辑态都用轻微 inset 底色表达可编辑性。
- 详情更多菜单位于“转写”标题行最右侧，顺序为 AI 讨论、音轨导出、单一“导出转写文本与笔记”（Markdown）、归档和删除；录音来源设置能力保留但不在此菜单暴露。AI 讨论与列表入口复用同一 Workspace 选择和新 Session 链路。
- 每条转写正文仍可点击跳转音频；条目 hover 或复制按钮获得键盘焦点时，右上角以绝对覆盖层出现独立胶囊“复制”，不得为按钮缩窄正文或预留静态空白，按钮左侧使用透明到按钮表面的短渐变遮罩保证文字可读。只复制该条正文，成功态显示“已复制”并在 3 秒后恢复；鼠标离开条目后无论是否仍在反馈期都立即隐藏，且不触发 seek。
- 现场笔记阅读态不为行内动作预留宽度；hover/focus 时以带短渐变遮罩的绝对覆盖层显示单一“更多”按钮，编辑与删除收进菜单。编辑态使用与底部笔记 composer 一致的透明线框表面，保存与取消始终随编辑态显示在输入框正下方，不依赖 textarea 焦点。
...
- 混选 text/audio Record 时“合并”保持置灰，禁用原因只通过该按钮的 hover/focus 提示表达，不向批量工具条插入额外长文案或改变菜单布局。
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 2.8.69 | 2026-08-29 | **Record 详情双栏与动作降噪**：现场笔记顶对齐右栏；详情菜单移入转写行并合并文稿导出；单条转写支持复制反馈；列表更多菜单移除重复 AI/播放入口，混选合并只保留禁用提示 |

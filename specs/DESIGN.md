...
通知 Bell 是 App Shell 的常驻底部入口，不受登录态或 Team Space UI 偏好隐藏；有未读时只在 16px Bell 右上角显示 6px `--accent-warm` 圆点，不显示数字。点击打开与 rail 工作区 hover 面板同材质的 fixed 非模态面板，展开/rail 时都从当前侧栏右侧起，宽度上限 380px、高度上限 620px，并与工作区 flyout、小助理 popover 和其它 close-layer 互斥；外部点击、Escape、Cmd+W 关闭并按入口恢复焦点。面板 header 固定标题、当前公开/账号 scope 与“全部已读”，正文独立滚动；加载、空态、局部错误和加载更多不改变外壳尺寸。

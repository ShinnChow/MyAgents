...
- `space_cloud::notifications` 是 App 进程级 Owner，不依赖 Team Space Tab、Session Sidecar 或 Registered Agent connector。它复用根模块的 build capability、optional user session、HTTP/client-context header 与 401 reauth transition，只调用一个 `/api/notifications` feed。

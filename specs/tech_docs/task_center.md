...
App Shell 只挂一个 `DispatchTaskDialog`：侧边栏和 Task Center 默认智能 Tab，Thought 派发默认手动 Tab。智能 Tab 与 Thought AI 讨论共用 Task discussion builder、runtime selection、required product Skill 和新 Chat launch；确认前不写 Task。手动创建只编辑完整 `task.md`，不再提供第二份验收输入；会话策略、触发前检测和结束条件只属于周期触发，立即执行与定时一次统一投影为 `new-session` 且不提交隐藏 trigger。既有 Task 的标签字段、列表过滤和编辑兼容能力保持不变。

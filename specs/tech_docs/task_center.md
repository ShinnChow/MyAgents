...
App Shell 只挂一个 `DispatchTaskDialog`：侧边栏和 Task Center 默认智能 Tab，text Record 派发默认手动 Tab。智能 Tab 与 Record AI 讨论共用 Task discussion builder、runtime selection、required product Skill 和新 Chat launch；确认前不写 Task。Record AI 讨论先选择工作区，再直接进入新的 AI discussion Session：text Record 把完整当前正文作为用户可见 query，audio Record 只传 `RecordStore` 保证已刷新且位于源 Record 根目录的 `content.md` 绝对路径。逐轮隐藏 reminder 只携带 workspace/candidates scope、Record identity/path 和 audio 文稿路径，不携带标签、discussion 临时目录或时间戳等无助于语义理解的字段。手动创建与 Task 编辑只编辑完整 `task.md`，不再提供第二份验收输入、简短描述或标签输入；Agent 工作区在高级配置之前选择，编辑时 `workspaceId + workspacePath` 必须由 TaskStore 在同一写锁内成对更新。会话策略、触发前检测和结束条件只属于周期触发，立即执行与定时一次统一投影为 `new-session` 且不提交隐藏 trigger / Session 选择。既有 Task 的 description、tags 和列表过滤仍保留为持久化兼容数据，但表单不再写回它们。

任务中心顶部的 Record 区统一列出 text/audio，并以 RecordStore change event 刷新；它只投影 Record lifecycle，不轮询或复制 Recording/Speech 状态。text Record 保留编辑、归档、批量删除和转 Task；audio Record 复用 text Record 的卡片边距与字阶：第一行是开始日期时间和稳定更多菜单，第二行是录音图标、标题与状态 tag，hover/focus 左侧出现“AI 讨论”，播放、归档、删除等低频动作进入更多菜单。点击主体打开顶部单实例 Record Tab，详情页从各自 authority 读取录音 snapshot、transcript/diarization 与 speaker overrides。Record 删除与 Task 删除是两条独立事务；已由 Record 派发的 Task 不因源 Record 删除而消失。

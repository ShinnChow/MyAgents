...
首次 observation 即使已经超过 deadline 也至少做一次有界真实读取。`pending` 只在剩余 grace 内继续等待；deadline、status read error 或未就绪状态只结算 dispatch released，基础 AI turn 继续。ready/released 在 owner 上只结算一次，后续 turn 不再为 dispatch 读 status；独立低频 observer 仍在同一 Runtime generation 内更新 live capability 与 tool catalog。用户取消仍取消 exact promotion；owner replacement 不属于 MCP released，旧 generator 必须 requeue 并退出。`runInjectedTurn()` 不再创建 MCP pre-persistence guard、promotion 二次 fence或 MCP 408/503；Goal/Task domain guard 的 claim / rollback / active-turn handoff 顺序保持原契约。

Live `setMcpServers()` 仍由 Query-generation mutation owner 单飞；promoted item、active turn 和 SDK command in-flight 都是 quiescence blocker。mutation claim 与 promotion 同步互斥：先 promotion 就锁存 restart，先 claim 就由 promotion 等待 mutation promise。真实 mutation 失败或 30 秒 timeout 仍意味着 transport map 不确定，旧 generator requeue 后隔离到 exact Query abort/replacement并退出；这条硬 fence 不与 startup soft grace 合并。`mcpServerStatus()` 在每个 observer tick 内单飞，status timeout 只标记该次 observation stale，不触发按 injected request 重建。External adapters 继续保持 runtime-native MCP ownership；仅 Managed Codex 的 MyAgents-injected config 采用下述同一 policy。
...
`AgentConfig.capabilitySelection` 是唯一持久化 authority；写开关时，当前 workspace 必须经唯一 `Project.agentId` 精确找到唯一 Agent。Runtime 读取不借用这条写入约束：owner 暂缺、重复或 selection 无法读取时采用无 disabled override 的默认集合，不能在普通消息入队前阻断 Session。`src/server/project-capabilities.ts` 统一扫描项目与 `~/.myagents` 候选，按 canonical name 执行 project > global winner，最后应用 disabled override。普通 Required System Skill 是 canonical name 的启用策略；`ProductSystemSkillRequirement` 是明确例外，它还携带 app-owned source identity、正文 digest 与 inventory revision，依赖该产品 Skill 的 turn 必须对 exact candidate 和当前 Runtime 投影 fail closed。其它 Skill 的同名、目录名差异或局部解析问题只淘汰对应候选，不得提升为整个 workspace / Session 不可用。设置页展示全部 winner；Chat Sidebar 与输入框只消费同一 Sidecar snapshot 的 enabled 集合。Rust slash scanner只保留无 Sidecar 的 Launcher 兼容读取，不解释项目 selection，也不在读取时改动 projection。
...
锁定的 Codex app-server `0.149.0` 中，`thread/compact/start` 与 `review/start` 分别提供原生压缩和审查 RPC，但两者都会创建 Codex 控制回合。当前仅 `compact` 已接入：Renderer 的 `/compact` 与上下文卡片按钮共用 `/api/session/compact`，route 只调用 SessionEngine facade；external Session owner 负责 idle admission、mutation lease、`chat:status`/`chat:system-status` 与排队消息恢复，Codex adapter 负责 RPC、control turn terminal 及隔离其 item/turn 事件，禁止把压缩写成用户/助手 transcript。RPC/timeout 等不确定失败会重启 runtime 进程边界，明确失败 terminal 则保留进程。`review` 尚未建立对应产品语义，继续隐藏；`context` 已由实时上下文指标承担只读展示但没有等价管理 RPC，`cost` 只有 token usage、没有费用语义，其余 Claude 系统指令没有可忠实映射的 Codex RPC。

Required System Skill 名单仍由 `src/shared/systemSkills.ts` 唯一决定；全局安装完整性只产生候选级诊断，项目 effective snapshot 的 `required` 由 winner 的 canonical name 决定。项目同名目录或目录 symlink 都保留 project provenance、服从既有 project > global 优先级并强制启用；目录名不参与普通 Skill 的真实性判断，也不得让单个 Skill 候选阻断整个 Session。投影 helper 只替换可证明指向 `~/.myagents` 的 managed symlink，项目文件与项目 symlink 保持原样；若 Project Skill 以任意目录名声明了相同 canonical name，同步入口移除对应 global link，使共享磁盘视图与 project winner 一致。单个候选或 link 无法读取/刷新时只记录日志，Session 继续启动；Builtin 与 Managed Codex 都从本次 admission 排除受影响 canonical。system-cli 兼容 Runtime 的普通 turn 仍可在局部投影失败时继续，但依赖 `ProductSystemSkillRequirement` 的 turn 必须拒绝已被 projection 标为 unavailable 的 exact app-owned Skill；不能把“Session 可用”误写成“依赖任务也可降级运行”。Managed Codex 还以启动后的 `skills/list` read-back 验证实际加载，缺失时同样只拒绝该依赖任务，普通对话和 Session 仍可用。
...
`runtimeSource:'system-cli'` 与 `runtimeSource:'managed-provider'` 都把 Skill 注入当成可选能力：同步、临时目录 materialization、`skills/extraRoots/set`、`skills/list` 或单个 Skill 解析失败只记录 warning，Codex Session 继续启动；但携带 `ProductSystemSkillRequirement` 的 dependent turn 会在 admission 时检查 exact candidate、inventory revision 与本次 projection，已知 unavailable 时 fail closed。Managed 仍走下节的精确 Product Extension projection，并追加 native loaded-Skill read-back；任何失败只拒绝依赖该 Skill 的 turn，不把普通对话或整个 Runtime 提升为失败。临时目录使用短序号作为文件系统名称，单个 Skill / Agent 链接或配置写入失败只跳过该候选。`skills/extraRoots/set` 与首次 `skills/list(forceReload:true)` 的 deadline 都是 30 秒；read-back 按 Skill 名称和 canonical source path 验收，其他 root 的同名 Skill 不算该候选成功。统一日志记录两个阶段的耗时、root/expected/visible/error 计数，并以 workspace/extra-root 相对路径及不可逆 message 摘要记录 Codex 返回的 `errors[]`，不持久化 parser 自由文本。parser warning 与缺失候选只进入日志，不出现在 Chat error banner。
...
Managed Codex 的**期望下载版本** authority 是 `src/shared/managed-codex-runtime.json`，当前目标 app-server 为 `0.149.0`；打包、后台更新与发布前 conformance 读取这份锁。当前 Runtime generation 的**有效执行版本**则来自安装器原子发布且通过签名与完整性校验的 `installed.json`，Session 启动只消费这份 identity，不得拿期望下载版本做相等性门控。这样后台下载期间，已安装的旧版本继续服务既有和新建 Session；更新成功后，后续新 generation 自然采用新版本。升级下载锁前必须重新生成 schema、完成 exact-binary conformance，并验证上一锁定版本创建的健康 native thread 可以在新版本继续 turn。`thread/start.dynamicTools` 的目录属于 native thread birth，resume 没有替换目录字段；但这不是 Product Session 兼容门槛。Session metadata 不保存 protocol version 或 Host catalog fingerprint，恢复时始终挂载当前 dispatcher。线程出生时已知且保持稳定语义的工具按当前 descriptor/schema/权限/generation 逐调用校验；已删除、未知或参数无效只失败该次工具调用，不使 Session 或整个 Host catalog 不可用。新增工具只保证在新 native thread 可见。不得以目录漂移或目标更新版本为由阻断基础 Runtime、清空 transcript、伪造空白 native thread、要求新 Product Session，或建立 MCP fallback、额外 Extension Host、第二套 Agent loop。
...
**Managed Codex readiness**：`initialize` 只证明 app-server 已建立；MyAgents 注入的 MCP 仍可能异步启动。Managed-provider launch config 给原生启动保留独立 60 秒 attempt bound；`CodexRuntime.startSession()` 在 `thread/start|resume` boundary 另行启动产品统一的 10 秒 dispatch grace，并持续观察 `mcpServer/startupStatus/updated` 与 `mcpServerStatus/list`。grace 到期只放行首个 `turn/start`，不会停止原生 attempt，也不会把 server 永久标为 degraded；`starting → cancelled → ready` 的迟到 ready 会更新中性 effective snapshot 与工具目录。MyAgents-owned 本地 stdio server 的应用级 admission queue 也计入这 10 秒；grant 迟到时 Managed Codex 在 active turn 结束后的 idle boundary 自动 replacement + prewarm，不自动重放已执行 turn。等待和 replacement 只覆盖 MyAgents 注入的 server name，用户自有 Codex MCP 不归此 owner。Process exit、thread/RPC failure 仍是真 Runtime failure，不能被 released 吞掉；`system-cli` launch config 与行为不变。
...
`startSession` 完成 `thread/start`（managed-provider 还需完成一次 ready / released dispatch settlement，live capability 继续独立观察）之后 **fire-and-forget**（不额外 block 首轮 turn）调用四个 Codex app-server RPC：

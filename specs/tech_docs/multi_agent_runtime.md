...
Managed Codex binary 的唯一版本 authority 是 `src/shared/managed-codex-runtime.json`，当前锁定 app-server `0.149.0`；启动 conformance、打包与安装都读取这份锁。升级下载锁前必须重新生成 schema、完成 exact-binary conformance，并验证上一锁定版本创建的健康 native thread 可以在新版本继续 turn。`thread/start.dynamicTools` 的目录属于 native thread birth，resume 没有替换目录字段；但这不是 Product Session 兼容门槛。Session metadata 不保存 protocol version 或 Host catalog fingerprint，恢复时始终挂载当前 dispatcher。线程出生时已知且保持稳定语义的工具按当前 descriptor/schema/权限/generation 逐调用校验；已删除、未知或参数无效只失败该次工具调用，不使 Session 或整个 Host catalog 不可用。新增工具只保证在新 native thread 可见。不得以目录漂移为由清空 transcript、伪造空白 native thread、要求新 Product Session，或建立 MCP fallback、额外 Extension Host、第二套 Agent loop。

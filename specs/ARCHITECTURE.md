...
可下载模型权重由 `SpeechModelPackManager` 子模块拥有，native Worker、sherpa adapter 和共享 ONNX Runtime 仍属于随 App 发布的受信任执行层。用户安装只访问固定的第一方签名 manifest 和其中编译期锁定的 content-addressed 第一方模型/许可镜像，不接受远端路径或 inventory；上游 GitHub URL 只存在于 release-only origin lock，不进入 App 运行时。远端 manifest 的原始 bytes 必须通过 App-updater Minisign trust-root 验签并原样持久化；App 与 Worker 再以拒绝未知字段的 typed source-lock 做完整语义 identity 比较，JSON 排版和 CRLF/LF 不属于资源身份，其余字段或数组顺序漂移全部拒绝。五个模型文件和五份许可文件在 private staging 中完成 regular-file、非执行权限、size 与 SHA-256 校验，再由真实 media Worker 最小加载 ASR/VAD/diarization 后发布版本目录并原子切换 `active.json`。安装不会为历史 Record 自动创建 job；没有 transcript 的历史音频只能由详情页“开始转录”显式提交。

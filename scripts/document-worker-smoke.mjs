...
const resourceRoot = resolve(
  process.argv[3] ??
    join(projectRoot, 'src-tauri', 'resources', 'document-processing', 'v1'),
);
if (!inputPath || !existsSync(inputPath) || !statSync(inputPath).isFile()) {
  throw new Error(
    'Usage: node scripts/document-worker-smoke.mjs <input-file> [resource-root]',
  );
}
...
if (!existsSync(workerPath) || !existsSync(manifestPath)) {
  throw new Error(
    `Prepared Worker resources are missing under ${resourceRoot}`,
  );
}
const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'));
const onnxRuntimePath = resolve(
  resourceRoot,
  manifest.files?.onnxRuntime?.path ?? '',
);
if (!manifest.files?.onnxRuntime?.path || !existsSync(onnxRuntimePath)) {
  throw new Error(
    'Prepared ONNX Runtime is missing from the verified resource manifest',
  );
}
...
writeFileSync(
  join(stagingPath, '.myagents-owner'),
...
function decodeFrames(bytes) {
  ...
  while (offset < bytes.length) {
    if (bytes.length - offset < 4)
      throw new Error('Worker returned a truncated frame prefix');
    ...
    messages.push(
      JSON.parse(bytes.subarray(offset, offset + length).toString('utf8')),
    );
    offset += length;
  }
  return messages;
...
try {
  ...
  child.stdout.on('data', (chunk) => {
    ...
    while (undecoded.length >= 4) {
      const length = undecoded.readUInt32BE(0);
      if (length === 0 || length > 1024 * 1024) {
        child.kill();
        return;
      }
      if (undecoded.length < length + 4) break;
      const message = JSON.parse(
        undecoded.subarray(4, length + 4).toString('utf8'),
      );
      messages.push(message);
      undecoded = undecoded.subarray(length + 4);
      if (message.type === 'ocr_lease_requested') {
        child.stdin.write(
          encodeFrame({
  ...
  const decodedMessages = decodeFrames(Buffer.concat(stdout));
  if (
    undecoded.length !== 0 ||
    JSON.stringify(decodedMessages) !== JSON.stringify(messages)
  ) {
    throw new Error('Worker smoke incremental frame decoding diverged');
  }
  ...
  if (
    ...
    !terminal.result?.detectedFormat ||
    !existsSync(documentPath)
  ) {
    throw new Error(
      `Worker smoke failed: ${JSON.stringify(terminal ?? messages)}`,
    );
  }
  console.log(
    JSON.stringify(
      {
        ...
        keptOutput:
          process.env.MYAGENTS_KEEP_SMOKE_OUTPUT === '1' ? scratch : undefined,
      },
      null,
      2,
    ),

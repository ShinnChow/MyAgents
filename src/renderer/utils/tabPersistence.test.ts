...
import { MAX_TABS, type ChatTab, type RecordTab, type Tab } from '@/types/tab';

function chatTab(over: Partial<ChatTab> = {}): ChatTab {
  return {
...
function recordTab(over: Partial<RecordTab> = {}): RecordTab {
  return {
...
describe('serializeTabs', () => {
  it('keeps only chat tabs with a real session + workspace', () => {
    const tabs: Tab[] = [
      ...
      { id: 'b', view: 'launcher', title: 'New Tab' },
      { id: 'c', view: 'settings', title: 'Settings' },
      { id: 'd', view: 'taskcenter', title: 'Tasks' },
  ...
  it('returns null when nothing is restorable', () => {
    const tabs: Tab[] = [{ id: 'b', view: 'launcher', title: 'New Tab' }];
    expect(serializeTabs(tabs, 'b')).toBeNull();
  });

  it('de-dupes by tab id (duplicate ids collide as React keys / owner ids)', () => {
    const tabs = [chatTab({ id: 'same', sessionId: 's1' }), chatTab({ id: 'same', sessionId: 's2' })];
  ...
  it('de-dupes Record tabs by recordId', () => {
    const state = serializeTabs(
      [recordTab({ id: 'record-a', recordId: 'same-record' }), recordTab({ id: 'record-b', recordId: 'same-record' })],
      'record-b',
    )!;
  ...
  it('caps at MAX_TABS', () => {
    const tabs = Array.from({ length: MAX_TABS + 5 }, (_, i) => chatTab({ id: `t${i}`, sessionId: `s${i}` }));
...
describe('deserializeTabs', () => {
  ...
  it('returns null on version mismatch', () => {
    const raw = JSON.stringify({
      version: 99,
      tabs: [{ id: 'a', agentDir: '/ws', sessionId: 's', title: 't' }],
      activeTabId: 'a',
    });
...
describe('hydratePersistedState', () => {
  ...
  it('hydrates Chat with pending Sidecar disposition and Record without a Session', () => {
    ...
    expect(tabs).toEqual([
      ...
      { id: 'b', view: 'record', title: 'B', recordId: 'record-b' },
    ]);
  });
});
...
describe('planRestoreTabs (Issue #309 — pill restore merge)', () => {
  const launcher: Tab = { id: 'launch-1', view: 'launcher', title: 'New Tab' };
  ...
  it('collapses multiple missing-Record fallbacks to one Task Center singleton', () => {
    const firstFallback: Tab = { id: 'missing-record-1', view: 'taskcenter', title: 'Records' };
    const secondFallback: Tab = { id: 'missing-record-2', view: 'taskcenter', title: 'Records' };
    const plan = planRestoreTabs(
      [launcher],
      { tabs: [firstFallback, secondFallback], activeTabId: secondFallback.id },
    );

    expect(plan).toEqual({ tabs: [firstFallback], activeTabId: firstFallback.id });
  });

  it('does not restore a missing-Record fallback beside an existing Task Center', () => {
    const existing: Tab = { id: 'tasks', view: 'taskcenter', title: 'Tasks' };
    const fallback: Tab = { id: 'missing-record', view: 'taskcenter', title: 'Records' };

    expect(planRestoreTabs([existing], { tabs: [fallback], activeTabId: fallback.id })).toBeNull();
  });
  ...
  it('returns null when every candidate tab is already open (all deduped)', () => {
    ...
    expect(
      planRestoreTabs([open], {
        tabs: [restored('r1', 's-1')],
        activeTabId: 'r1',
      }),
    ).toBeNull();
  });

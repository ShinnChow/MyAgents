...
- **macOS 双架构与 Windows 首次构建恢复可靠**：同时构建 Apple Silicon/Intel 时，各架构只使用自己的本地推理资源；Windows 初始化也不再因上游源码包中的无关链接文件而中断。
- **Record 搜索结果更容易判断命中内容**：音频记录会直接展示搜索命中的文本片段，批量操作入口也与当前界面保持一致。
- **录音权限与设备变化恢复更可靠**：权限检查、输入设备变化和短暂采集异常都有明确状态与恢复路径，已写入的录音归档不会因后续处理失败而丢失。
